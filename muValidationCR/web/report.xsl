...
-->
                ...
                <xsl:if test="count(Report/Results[@severity='errors' and @specification='cr']/issue) > 0">
                    <h3>IG for Ambulatory Healthcare Provider Reporting to Central Cancer Registries errors:</h3>
                    <xsl:for-each select="Report/Results[@severity='errors' and @specification='cr']/issue">
                        <table cellpadding="3" cellspacing="3">
                            <xsl:attribute name="bgcolor">
                                <xsl:choose>
                                    <xsl:when test="(position() mod 2 = 1)">
